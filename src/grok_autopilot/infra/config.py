"""
Grok Autopilot — Configuration (pydantic-settings)
=====================================================
Settings loaded with priority chain (highest → lowest):
    1. Environment variables (QODER_*, .env)
    2. User config (~/.grok-autopilot/config.json)
    3. Built-in defaults

Usage:
    from grok_autopilot.config import settings      # Settings singleton
    from grok_autopilot.config import WORKER_URL     # Module-level re-exports

CLI configuration:
    grok-autopilot config show                       # Show all settings
    grok-autopilot config set worker-url https://...  # Set a value
    grok-autopilot config reset                       # Reset to defaults
"""
...
class Settings(BaseSettings):
    """
    Centralized configuration for Grok Autopilot.
    ...
    def _load_user_config(cls) -> dict:
        """Load user config from ~/.grok-autopilot/config.json."""
        try:
    ...
    # ── Grok URLs (constants, rarely need override) ──────────────────────
    grok_signup_url: str = Field(
        default="https://grok.com/users/sign-up",
        description="Grok sign-up page URL",
    )
    grok_signin_url: str = Field(
        default="https://grok.com/users/sign-in",
        description="Grok sign-in page URL",
    )
    grok_login_url: str = Field(
        default="https://grok.com/device/selectAccounts",
        description="Grok device login URL",
    )
    grok_device_token_url: str = Field(
        default="https://openapi.grok.sh/api/v1/deviceToken/poll",
        description="Grok device token polling endpoint",
    )
    grok_userinfo_url: str = Field(
        default="https://openapi.grok.sh/api/v1/userinfo",
        description="Grok user profile endpoint",
    )
    ...
    # ── Captcha ──────────────────────────────────────────────────────────
    twocaptcha_key: str = Field(
        default=os.environ.get("TWOCAPTCHA_KEY", ""),
        description="2captcha API key for AlibabaTaskProxyless solving",
    )
    ...
    credentials_file: Path = Field(
        default=Path("grok_accounts.json"),
...
# Grok URLs
QODER_SIGNUP_URL = settings.grok_signup_url
QODER_SIGNIN_URL = settings.grok_signin_url
QODER_LOGIN_URL = settings.grok_login_url
QODER_DEVICE_TOKEN_URL = settings.grok_device_token_url
QODER_USERINFO_URL = settings.grok_userinfo_url
