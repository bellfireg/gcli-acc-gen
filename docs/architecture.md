...
Grok Autopilot automates the full lifecycle of Grok account registration:
...
1. **Navigate** → Open Grok signup or OAuth URL in Camoufox
...
```python
from grok_autopilot.config import settings
...
```python
from grok_autopilot.errors import (
    GrokAutopilotError,    # catch-all base
